...
import { ReportStatus, ScoutingDetailsDto, ScoutingSummaryDto } from '../../../../core/models/scouting.model';
import { finalize } from 'rxjs/operators';

@Component({
  selector: 'app-scouting-ops',
  standalone: true,
  imports: [
    CommonModule,
    TranslateModule,
  ...
  templateUrl: './scouting-ops.component.html',
  styleUrl: './scouting-ops.component.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ScoutingOpsComponent {
  ...
  // Use a writable signal for table data to allow updates from loadData
  ...
  selectedDetails = signal<ScoutingDetailsDto | null>(null);
  ...
  openDetails(item: ScoutingSummaryDto): void {
    this.detailsVisible = true;
    this.selectedDetails.set(null);
    this.detailsLoading.set(true);
    this.detailsError.set(null);
    this.scoutingService
      .getScoutingReportById(item.id)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .pipe(finalize(() => this.detailsLoading.set(false)))
      .subscribe({
        next: (d) => this.selectedDetails.set(d),
        error: () => {
          this.selectedDetails.set(null);
          this.detailsError.set(this.translate.instant('common.loadFailed'));
        },
      });
  }
}
