...
@Component({
  ...
  template: `
    <div class="p-6">
      <h2 class="text-2xl font-bold mb-6 text-slate-800">{{ 'dashboard.controlPanel.scoutingOps.title' | translate }}</h2>
                ...
                <tr class="hover:bg-slate-50 border-b border-slate-100 transition-colors">
                    ...
                    <td class="p-4">
                        <span class="px-2 py-1 rounded-full text-xs font-bold"
                             ...
                             <!-- Note: Ideally mapped, but for now we display status. If status is an EN/Key enum, we should translate it too.
                                  Assuming 'Completed' matches the key. For safety, I'll try to map it or just leave as is if backend sends display string.
                                  User verification instruction implied verifying everything. -->
                            {{ op.status }} 
                        </span>
                    </td>
                </tr>
...
export class ScoutingOpsComponent {
  ...
  ops = toSignal(this.mockService.getScoutingOps());
}
